...
def substructure_preserver(mol):
    ...
    if mol.HasSubstructMatch(rdkit.Chem.MolFromSmarts('COc(c1)c(C)cc2c1ncnc2Nc3cc(Cl)c(F)cc3')) == True:
...
def mutate_selfie(selfie, max_molecules_len, write_fail_cases=False):
    ...
    return (selfie_mutated, smiles_canon)
smi     = 'COc(c1)c(OCCCN2CCOCC2)cc3c1ncnc3Nc4cc(Cl)c(F)cc4'
...
smi_arranged.reverse()
canon_smi_ls=smi_arranged
print('smi_arranged')
print(smi_arranged)
print('canon_smi_ls')
print(canon_smi_ls)
#mols_ = [Chem.MolFromSmiles(x) for x in smi_arranged]
fhout=open('generative.smi', 'w')
for l in smi_arranged:
    fhout.write(l +'\n')
fhout.close()
